  ...
  function onSubmit(input: MotorSchema) {
    ...
    startTransition(async () => {
      try {
        ...
        console.log('Prepared update data:', formData);
...
console.log(task?.files)
    ...
    return (
      <Sheet {...props} modal={true}>
        <SheetContent className="flex flex-col gap-6 overflow-y-scroll">
          ...
          <TaskForm<MotorSchema> form={form} onSubmit={onSubmit} isPending={loading} initialFiles={task?.files! as FileWithPreview[]} fileUploadRef={fileUploadRef} >
  ...
  <DrawerContent className="flex flex-col  ">
    ...
    <DrawerTitle className=" text-xl">Update<Button type="button"   variant={"outline"} className=" ml-2  px-2.5 text-base">{task?.name}</Button> </DrawerTitle>
      ...
      <TaskForm<MotorSchema>  form={form} onSubmit={onSubmit} isPending={loading} fileUploadRef={fileUploadRef}  initialFiles={task?.files!  as FileWithPreview[]}   >
